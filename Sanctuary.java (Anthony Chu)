...
public class Sanctuary {
    ...
    public int rescue(String species, int num) {
        ...
        if (!sanctuary.containsKey(species)) {
            if (this.getTotalSpecies() == this.getMaxSpecies() || this.getTotalAnimals() == this.getMaxAnimals()) {
                return num;
            }
            int different = this.getMaxAnimals() - this.getTotalAnimals();
            if (num > different) {
                sanctuary.put(species, different);
        ...
        int different = this.getMaxAnimals() - this.getTotalAnimals();
        if (num > different) {
            sanctuary.put(species, this.countForSpecies(species) + different);
            return num - different;
        }
        sanctuary.put(species, this.countForSpecies(species) + num);
        return 0;
    }
